...
void ed_fm_simulate(double dt)
{
	...
	*/
	...
	s_engine->setThrottle(s_input->throttle()); // Maybe do this better directly in set_command method
	...
	/* Disable all interface releates stuff for now.
...
{	
	...
	// Prop state
	drawargs[PROPELLER_STATE].f = s_engine->getPropellerState();	
...
double ed_fm_get_param(unsigned index)
{
	...
	case ED_FM_PISTON_ENGINE_1_MANIFOLD_PRESSURE:
		break; // Provide a useful value here

		// Propeller
	case ED_FM_PROPELLER_0_RPM:    // propeller RPM , for helicopter it is main rotor RPM
		return s_engine->getPropellerRPM(); // Reduction Gear 3:2
	...
	case ED_FM_PROPELLER_0_TILT:
		return s_engine->getPropellerState();
